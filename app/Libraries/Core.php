...
class Core
{
     ...
     */
    public function setResponse( $type = 'not_found',$message = "Not Found", $data = [],  $ispaginated = false, $is_array = FALSE, $code = null ){
        ...
        if($ispaginated == true){
            ...
            $response =  [ "message" => $message, "status"  => $status , 'metadata' => $data ];
        } else{
            $response =  [ "message" => $message, "status"  => $status ,$body => $data ];
        }
